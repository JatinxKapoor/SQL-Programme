...
select department,first_name,emp_id,avg(salary) from emp_info
group by department,first_name,emp_id
having avg(salary)> "60,000.00"  order by avg(salary) desc limit 3;
...
/* Practice*/
select department,emp_id,first_name,last_name,avg(salary) from emp_info
group by department,emp_id,first_name,last_name;
select count(emp_id) from emp_info where hire_date>'2019-12-31';
select department , sum(salary) as total_salary from emp_info 
group by department;
select salary from emp_info where department='Finance' order by salary desc limit 1;
select emp_id,first_name,last_name,salary from emp_info order by salary desc limit 3;
select department,min(salary) from emp_info 
group by department order by min(salary) asc;
select department , count(emp_id) from emp_info 
group by department ;
select emp_id,first_name,last_name,hire_date,avg(salary)  from emp_info 
group by emp_id,first_name,last_name,hire_date
having hire_date <=('2020-1-1');
select hire_date,first_name,last_name from emp_info where department="it" order by hire_date desc limit 1;
select emp_id,first_name,last_name,hire_date,sum(salary) from emp_info
group by emp_id,first_name,last_name,hire_date
having hire_date>'2019-1-1';
select department,emp_id,salary from emp_info where department='hr' order by salary limit 1;
select department,sum(salary) from emp_info
group by department 
order by department
limit 2;
